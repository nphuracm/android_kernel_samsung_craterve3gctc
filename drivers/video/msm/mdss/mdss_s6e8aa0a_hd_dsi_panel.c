...
int mdss_dsi_s6e8aa0a_panel_reset(struct mdss_panel_data *pdata, int enable)
{
...
//			gpio_set_value((ctrl_pdata->disp_en_gpio), 0);
		...
		gpio_set_value(ctrl_pdata->disp_en_gpio_2_2v, 0);
		gpio_set_value((ctrl_pdata->rst_gpio), 0);
	}
	return 0;
}
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
	...
	if(msd.mfd->resume_state == MIPI_SUSPEND_STATE)
		return 0;
...
static int __init detect_lcd_panel_vendor(char* read_id)
{
	...
	printk("detect_lcd_panel_vendor lcd_id3 = %x\n",lcd_id3);
