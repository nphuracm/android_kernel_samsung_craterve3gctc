...
#if defined(CONFIG_MACH_CS03_SGLTE) || defined(CONFIG_MACH_VICTOR_CHN_SGLTE) || defined(CONFIG_MACH_VICTOR_CHN_SGLTE_CU)
				chg_imax_ma = 700;
#else
				chg_imax_ma = 1000;
#endif
...
static void sec_bat_monitor(struct work_struct *work)
{
	...
	static u8 btm_count;
#endif
	#if defined(CONFIG_MACH_CS03_SGLTE) || defined(CONFIG_MACH_Q7_CHN_SGLTE) || defined(CONFIG_MACH_VICTOR_CHN_SGLTE) || defined(CONFIG_MACH_VICTOR_CHN_SGLTE_CU)
	int rc;
	u8 buck_sts = 0;
	#endif
	...
	if ((chip->cable_type == CABLE_TYPE_USB ||
		...
		(chip->batt_health == POWER_SUPPLY_HEALTH_GOOD)) {
			...
			#else
#if defined(CONFIG_MACH_MS01_LTE) || defined(CONFIG_MACH_MS01_CHN_SGLTE) || defined(CONFIG_MACH_CRATERVE_CHN_CTC)
